...
def MD_sim_charge(Q_VALS, R_eps_theta_k, optimizer='None'):
    ...
    lammps_setup = MDSetup(
        system_setup="config/setup_mechanical_charge_pcff1.yaml",
        ...
        if equil_job_count() <= 0:
            ...
            for charge_set in CHARGE_SETS:
                for tob in TOB_STRUCTURES:
                    try:
                        initial_systems = [
                            f"runs/charge_optimization/pcff/{tob}/{charge_set}/equilibration/temp_298.1_pres_1.0/copy_0/01_npt/npt.data"
                        ]
    ...
    while True:
        if (equil_job_count()+deform_job_count()) <= 0:
            ...
            OUTPUT_DIR = 'runs/charge_optimization/pcff'
