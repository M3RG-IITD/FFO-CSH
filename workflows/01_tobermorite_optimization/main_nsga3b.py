...
pd.DataFrame(np.hstack([X_test_orig, y_test_orig]), columns=df_full.columns).to_csv(f'../../data/training/models/NSGA3b/data_test.csv', index=False)
...
for AL_run in range(10):
    ...
    with open(f'../../data/training/models/NSGA3b/test_accuracy_{AL_run}.txt', 'w') as f:
        f.write(f'R2 score: {test_r2}\n')

    # save the model
    torch.save(model.state_dict(), f'../../data/training/models/NSGA3b/model_{AL_run}.pth')
    # # load the model
    # model.load_state_dict(torch.load(f'../../data/training/models/NSGA3b/model_{AL_run}.pth'))
    ...
    df_new_nsga3b = df_new_nsga3b.dropna()
    
    # === 8. Evaluate match ===
    md_prop_pred = df_new_nsga3b.values[:,4:]
    ...
    X_train = np.vstack([X_train, df_new_nsga3b.values[:, :4]])
    y_train = np.vstack([y_train, df_new_nsga3b.values[:, 4:]])

    # Save cumulative data
    df_updated = pd.DataFrame(np.hstack([X_train, y_train]))
    df_updated.columns = df_full.columns
    df_updated.to_csv(f'../../data/training/models/NSGA3b/data_{AL_run}.csv', index=False)
