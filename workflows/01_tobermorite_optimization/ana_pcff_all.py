...
ll = os.listdir('runs/pcff_lj_params/Tob14')
ll_clean = [x for x in ll if x.startswith('LJ_set_')]
# sort_ll = sorted(ll_clean, key=lambda x: int(x.split('_')[2]))


sort_ll = ['LJ_set_22','LJ_set_165','LJ_set_245_NSGA2','LJ_set_436_ocx_ktheta']
# sort_ll = ['LJ_set_74']
# === 1. Density Analysis ===
den_results = {tob: [] for tob in TOB_STRUCTURES}

for charge_set in sort_ll:
...
OUTPUT_DIR = 'runs/pcff_lj_params'
pickle_path = f"{OUTPUT_DIR}/all_density_{timestamp}.pkl"
csv_path = f"{OUTPUT_DIR}/all_density_{timestamp}.csv"
...
# mean_cols = ['D_11', 'D_11H', 'D_14']
# mask = df[mean_cols].dropna().index
# df_full_masked = df.loc[mask]
df_full_masked = df[mean_cols].apply(pd.to_numeric, errors='coerce')
target_y = np.array([2.46, 2.39, 2.23, 680, 325, 635, 71, 55.35, 47])
# target_y = np.array([2.46, 2.39, 2.23])
...
df_full_new.insert(0, 'LJ_set', sort_ll)
