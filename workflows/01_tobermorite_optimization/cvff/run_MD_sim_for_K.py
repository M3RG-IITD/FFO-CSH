...
def MD_sim_CVFF(suggested_params, optimizer='None'):
    ...
    TEMPLATE_FRC = os.path.join(BASE_DIR, "cvff_template.frc")
    TOB_DIRS = ["Tob11_car_mdf", "Tob11H_car_mdf", "Tob14_car_mdf"]

    lj_dir = [d for d in os.listdir(f"{BASE_DIR}/../CVFF") if os.path.isdir(os.path.join(BASE_DIR, "../CVFF", d)) and d.startswith("LJ_set_")]
    ...
    if lj_dir:
        ...
        count1 = 0  # default if no such directories exist
    ...
    for i, (sc_r, sc_eps, oc_r, oc_eps, ocx_r, ocx_eps, sos_ktheta, oso_ktheta, so_kr) in enumerate(zip(*suggested_params.T)):
        ...
        # angle and bond parameters
        sos_ktheta = sos_ktheta #100.0
        oso_ktheta = oso_ktheta #100.0
        so_kr = so_kr #350.0
        
        i += count1
        param_dir = os.path.join(BASE_DIR, f"../CVFF/LJ_set_{i+1}_{optimizer}")
        ...
        print(f"🔧 MD simulation with LJ 12-6 for LJ_set_{i+1}_{optimizer} using optimizer: {optimizer}")
        ...
        for tob_dir in TOB_DIRS:
            ...
            for prefix in prefixes:
                ...
                cmd = ["msi2lmp", output_prefix, "-class", "1", "-frc", os.path.join(param_dir, f"modified_LJ_{i+1}.frc"), "-ignore"]
    ...
    lammps_setup = MDSetup(
        system_setup="../config/setup_mechanical_cvff.yaml",
    ...
    for lj_set in LJ_SETS:
        for tob_structure, data_file in TOB_STRUCTURES.items():
            ...
            lammps_setup.prepare_simulation(
                ...
                initial_systems=[f"../../../data/structures/lj_sets/CVFF/{lj_set}/{tob_structure}/{data_file}"],
                input_kwargs={},
                copies=0,
                off_set=0,
            )
    ...
    while True:
        if equil_job_count() <= 0:
            ...
            for lj_set in LJ_SETS:
                try:
                    for tob_structure, data_file in TOB_STRUCTURES.items():
                        initial_systems = [f"runs/cvff_lj_params/{tob_structure}/{lj_set}/equilibration/temp_298.1_pres_1.0/copy_0/01_npt/npt.data"]
    ...
    while True:
        if (equil_job_count()+deform_job_count()) <= 0:
            ...
            with open(f"runs/cvff_lj_params/data_LJ_{count1}_{count2}_{timestamp}.pkl", 'wb') as f:
                results = {
            ...
            df_full_new.to_csv(f"runs/cvff_lj_params/data_LJ_{count1}_{count2}_{timestamp}.csv", index=False)
            print(f"✅ All jobs completed and data saved as runs/cvff_lj_params/data_LJ_{count1}_{count2}_{timestamp}.csv")
            return df
