...
pd.DataFrame(np.hstack([X_test_orig, y_test_orig]), columns=df_full.columns).to_csv(f'ML_Pipeline/models/BO/data_test.csv', index=False)
...
for AL_run in range(10):
    ...
    with open(f'ML_Pipeline/models/BO/test_accuracy_{AL_run}.txt', 'w') as f:
        f.write(f'R2 score: {test_r2}\n')

    # save the model
    torch.save(model.state_dict(), f'ML_Pipeline/models/BO/model_{AL_run}.pth')
    # # load the model
    # model.load_state_dict(torch.load(f'ML_Pipeline/models/BO/model_{AL_run}.pth'))
    ...
    # === 7. Run MD simulation ===
    df_new_bo = MD_sim(np.atleast_2d(x_opt_bo), optimizer= 'BO')
    df_new_bo = df_new_bo.dropna()
    
    # === 8. Evaluate match ===
    md_prop_pred = df_new_bo.values[:,4:]
    ...
    X_train = np.vstack([X_train, df_new_bo.values[:, :4]])
    y_train = np.vstack([y_train, df_new_bo.values[:, 4:]])

    # Save cumulative data
    df_updated = pd.DataFrame(np.hstack([X_train, y_train]))
    df_updated.columns = df_full.columns
    df_updated.to_csv(f'ML_Pipeline/models/BO/data_{AL_run}.csv', index=False)


# 'output_tob_all/pcff_lj_params/Tob11/LJ_set_149_BO/SE/bulk/temp_298.1_pres_1.0/copy_0/LOG.o'
# 'output_tob_all/pcff_lj_params/Tob11/LJ_set_149_CMA/SE/bulk/temp_298.1_pres_1.0/copy_0/LOG.o'
