...
# === 1. Load Data ===
df_full = pd.read_csv('ML_Pipeline/data/training_data_145.csv')
...
pd.DataFrame(np.hstack([X_test_orig, y_test_orig]), columns=df_full.columns).to_csv(f'ML_Pipeline/models/CMA/data_test.csv', index=False)
...
for AL_run in range(10):
    ...
    with open(f'ML_Pipeline/models/CMA/test_accuracy_{AL_run}.txt', 'w') as f:
        f.write(f'R2 score: {test_r2}\n')

    # save the model
    torch.save(model.state_dict(), f'ML_Pipeline/models/CMA/model_{AL_run}.pth')
    # # load the model
    # model.load_state_dict(torch.load(f'ML_Pipeline/models/CMA/model_{AL_run}.pth'))

    # === 6. Inverse Design ===
    target_y = [2.46, 2.39, 2.23, 680, 325, 635, 71, 55.35, 47]


    x_opt_cma, loss_cma = inverse_design_cma(
        model=model,
        ...
        popsize=30, # Population size per iteration
        sigma0=0.2 # Initial search spread (step size) CMA-ES starts from x0 ± sigma0 * N(0, I) (Gaussian sampling)
    )
    
    # === 7. Run MD simulation ===
    df_new_cma = MD_sim(np.atleast_2d(x_opt_cma), optimizer= 'CMA')
    df_new_cma = df_new_cma.dropna()
    
    # === 8. Evaluate match ===
    md_prop_pred = df_new_cma.values[:,4:]
    ...
    X_train_n = np.vstack([X_train, df_new_cma.values[:, :4]])
    y_train_n = np.vstack([y_train, df_new_cma.values[:, 4:]])

    # Save cumulative data
    df_updated = pd.DataFrame(np.hstack([X_train_n, y_train_n]))
    df_updated.columns = df_full.columns
    df_updated.to_csv(f'ML_Pipeline/models/CMA/data_{AL_run}.csv', index=False)
